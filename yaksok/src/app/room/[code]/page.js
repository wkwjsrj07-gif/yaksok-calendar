...
export default function RoomPage() {
  ...
  const handleJoin = async () => {
    ...
    const { data: pt, error: err } = await supabase
      .from('participants')
      .insert({ room_id: room.id, name: name, token: token })
      .select()
      .single();
  ...
  const handleDateClick = (dateStr) => {
    ...
    if (forDate.length === 0) {
      ...
      setPopup({ type: 'info', date: dateStr, names: names, canCancel: !!mine });
    }
  };
        ...
        <div style={S.overlay} onClick={() => setPopup(null)}>
          <div style={S.popupCard} onClick={e => e.stopPropagation()}>
            ...
            {popup.type === 'info' && <>
              ...
              {popup.canCancel && <button onClick={() => toggleDate(popup.date)} style={{...S.popupBtn,background:'#22c55e',color:'#fff',marginBottom:8}}>내 약속 취소하기</button>}
